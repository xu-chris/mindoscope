...
function buildMindmap(hash, zoomDuration) {
  ...
  function drawNodeList(root) {
    ...
    function renderTreelist(data) {
      ...
      var tree = d3.layout.tree(),
          ul = $treelistContainer.append("ul").classed("treelist",true),
          nodes = tree.nodes(data),
          nodeEls = ul.selectAll("li.item").data(nodes);
      //list nodes
    ...
    // tree link nodes
      ...
      var svgContainer = $treelistContainer
        .append("svg")
  ...
  function reset() {
    ...
    d3.select("#treelist").select('ul').remove();
    d3.select("#treelist").select('svg').remove();
    closeSidebar();
    closeSettings();
    d3.select('#searchterm')
        .classed('show',false);
